...
socket.on("playersJoined", function(joinInfo){//....in game.html.............................(1)

	playerData = joinInfo
	if (playerData.roomType == "private"){
		document.getElementById("roomId").innerHTML = "your Game Code is: " + joinInfo.roomId
	}
	document.getElementById("player").innerHTML = "Your Symbole is: " + joinInfo.letter
	sandwichWithSpinners("searching", "please wait")//adding spinner

})
//???????????????????????????????????????????????????????///

socket.on("playerLogedIn", function(){
	giveAcess()
})
...
function checkTurn(){// after this it goes to click event
	var turnText;
	if (yourTurn){
		clearInterval(loop)
		turnText = "Your Turn"
		document.getElementById("turn").innerHTML = turnText
		document.getElementById("turn").className = "It is your Turn"
	}else{
		turnText = " Oponnent's Turn "
		document.getElementById("turn").className = "It is no tYour Turn"
